...
export class Assignment3 extends Scene {
  constructor() {
    ...
    for (let i = 0; i < this.number_of_buildings; i += 2) {
      ...
      temp_right = temp_right.times(
        Mat4.translation(10, temp_scale_right * 1.8, 90 - i * 5),
      );
  ...
  make_control_panel() {
    ...
    this.key_triggered_button(
      ...
      () => {
        ...
        this.cary_speed = Math.min(0.2, this.cary_speed);
      },
      '#6E6460',
      () => {
        //release event
  ...
  display(context, program_state) {
    ...
    car_hit_box = car_hit_box.times(Mat4.scale(0.7, 0.6, 1.5));
    ...
    for (let i = 0; i < 4; i++) {
      ...
      this.obsticle_transforms[i * 4 + 1] = this.obsticle_transforms[i * 4 + 1]
        .times(
          Mat4.translation(
            4 * Math.sin((t * ((i * 4 + 1) % 10)) / 3),
            1.5,
            100 - (i * 4 + 1) * 10,
          ),
        )
        .times(Mat4.rotation(Math.PI / 4, 1, 0, 0));
      this.obsticle_transforms[i * 4 + 1] = this.obsticle_transforms[
        i * 4 + 1
      ].times(Mat4.rotation(t, 0, 1, 0));
      let obsticleAABB = Get_Dimensions_Of_Collision_Box(
        Object_to_World_Space(
          this.obsticle_transforms[i * 4 + 1],
      ...
      this.obsticle_transforms[i * 4 + 2] = this.obsticle_transforms[i * 4 + 2]
        .times(
          Mat4.translation(
            4 * Math.sin((t * ((i * 4 + 2) % 10)) / 3),
            0.75,
            100 - (i * 4 + 2) * 10,
          ),
        )
        .times(Mat4.rotation(Math.PI, 0, 1, 0));
      let lion_modified_hitbox = this.obsticle_transforms[i * 4 + 2].times(
        Mat4.scale(0.4, 1, 1.5),
      );
      obsticleAABB = Get_Dimensions_Of_Collision_Box(
        Object_to_World_Space(
          lion_modified_hitbox,
          this.shapes.hitbox.arrays.position,
        ),
      );
      if (checkCollision(carAABB, obsticleAABB)) {
      ...
      this.shapes.lion.draw(
        context,
        program_state,
        this.obsticle_transforms[i * 4 + 2],
        this.materials.lion,
      );
      if (this.show_collision_boxes)
        this.shapes.hitbox.draw(
      ...
      this.obsticle_transforms[i * 4 + 3] = this.obsticle_transforms[i * 4 + 3]
        .times(
          Mat4.translation(
            4 * Math.sin((t * ((i * 4 + 3) % 10)) / 3),
            1.5,
            100 - (i * 4 + 3) * 10,
          ),
        )
        .times(Mat4.rotation(Math.PI / 4, 1, 0, 0));
      this.obsticle_transforms[i * 4 + 3] = this.obsticle_transforms[
        i * 4 + 3
      ].times(Mat4.rotation(t, 0, 1, 0));
      obsticleAABB = Get_Dimensions_Of_Collision_Box(
        Object_to_World_Space(
          this.obsticle_transforms[i * 4 + 3],
          this.shapes.hitbox.arrays.position,
        ),
      );
      if (checkCollision(carAABB, obsticleAABB)) {
      ...
      this.shapes.teapot.draw(
        context,
        program_state,
        this.obsticle_transforms[i * 4 + 3].times(rotMatrix),
        this.materials.teapot,
      );
      if (this.show_collision_boxes)
        this.shapes.hitbox.draw(
          context,
      ...
      this.obsticle_transforms[i * 4 + 4] = this.obsticle_transforms[
        i * 4 + 4
      ].times(
        Mat4.translation(
          4 * Math.sin((t * ((i * 4 + 4) % 10)) / 3),
          0.25,
          100 - (i * 4 + 4) * 10,
        ),
      );
      obsticleAABB = Get_Dimensions_Of_Collision_Box(
        Object_to_World_Space(
          this.obsticle_transforms[i * 4 + 4],
          this.shapes.hitbox.arrays.position,
        ),
      );
      if (checkCollision(carAABB, obsticleAABB)) {
    ...
    wall_transform_start = wall_transform_start.times(
      Mat4.translation(0, 0, 101),
    );
    wall_transform_start = wall_transform_start.times(Mat4.scale(12, 5, 1));
    this.shapes.wall.draw(
      context,
      program_state,
      wall_transform_start,
      this.materials.wall,
    );

    this.shapes.sidewalls.arrays.texture_coord.forEach((v, i, l) => {
      v[0] *= 60;
      v[1] *= 1;
    });
    let wall_transform_left = Mat4.identity();
    wall_transform_left = wall_transform_left.times(
      Mat4.translation(-12, 0, 0),
    );
    wall_transform_left = wall_transform_left.times(
      Mat4.rotation(Math.PI / 2, 0, 1, 0),
    );
    wall_transform_left = wall_transform_left.times(Mat4.scale(100, 5, 1));
    this.shapes.sidewalls.draw(
      context,
      program_state,
      wall_transform_left,
      this.materials.wall,
    ...
    wall_transform_right = wall_transform_right.times(
      Mat4.translation(12, 0, 0),
    );
    wall_transform_right = wall_transform_right.times(
      Mat4.rotation(Math.PI / 2, 0, 1, 0),
    ...
    this.shapes.sidewalls.draw(
      context,
      program_state,
      wall_transform_right,
      this.materials.wall,
    );

    //finish line
    this.shapes.finishline.arrays.texture_coord.forEach((v, i, l) => {
      v[0] *= 10;
      v[1] *= 1;
    });
    let finish_line_transformation = Mat4.identity();
    finish_line_transformation = finish_line_transformation.times(
      Mat4.scale(10, 0.25, 1),
    );
    finish_line_transformation = finish_line_transformation.times(
      Mat4.translation(0, -1.9, -80),
    );
    let finish_line_hitbox = finish_line_transformation;
    finish_line_hitbox = finish_line_hitbox.times(Mat4.translation(0, 1, 0));
    let finish_lineAABB = Get_Dimensions_Of_Collision_Box(
      Object_to_World_Space(
        finish_line_hitbox,
        this.shapes.hitbox.arrays.position,
      ),
    );
    this.shapes.finishline.draw(
      context,
      program_state,
      finish_line_transformation,
      this.materials.finishline,
    );
